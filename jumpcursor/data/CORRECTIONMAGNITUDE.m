...
% Get the trial types from block_name
blockNames = wl.TrialData.block_name;

% Find the indices of valid trials (only 'SlowJumps' or 'FastJumps')
validTrialsIdx = strcmp(blockNames, 'SlowJumps') | strcmp(blockNames, 'FastJumps');

% Filter data to include only the valid trials
num_valid_trials = sum(validTrialsIdx);

% Preallocate arrays for valid trials
correctionX = NaN(1, num_valid_trials);
jumpDistances = NaN(1, num_valid_trials);
isFastTrial = false(1, num_valid_trials);

% Initialize index for valid trials
validIndex = 1;

% Loop through all trials, but only process valid ones
for trial = 1:num_trials
    if validTrialsIdx(trial)
        % Extract timestamps for the trial
        timeStamps = wl.TimeStamp(trial, 1:wl.Samples(trial));
        ...
        % Calculate correction relative to the target (in x-direction only)
        finalHandPos = [x(end), y(end)];  % Final hand position
        correctionX(validIndex) = finalHandPos(1) - target_pos(1);  % Signed correction along the x-axis
        
        % Extract jump distance for the trial
        jumpDistances(validIndex) = wl.TrialData.JumpDistance(trial);
        
        % Determine if the trial is a fast trial
        isFastTrial(validIndex) = strcmp(blockNames{trial}, 'FastJumps');
        
        % Increment index for valid trials
